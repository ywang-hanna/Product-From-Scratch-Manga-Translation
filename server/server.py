...
def images(book,size,page):
    if size == 'regular': return redirect(url_for('static',filename='images/'+book+'/'+page+'.jpg'))
    elif size == 'large': return redirect(url_for('static',filename= 'images/'+book+'/'+page+'-large.jpg'))
    elif size == 'thumb': return redirect(url_for('static',filename= 'images/'+book+'/'+page+'-thumb.jpg'))
    elif size == 'regions': return redirect(url_for('static',filename= 'images/'+book+'/'+page+'-regions.json'))
...
def home():
    ...
    return render_template('home.html', loginForm=login_form,
                           registerForm=register_form, login=login,
                           register=register)
...
class LoginForm(FlaskForm):
    ...
    userpassword = PasswordField('Password',
                                 [DataRequired()],
                                 id='passwordLogin')
...
def checkRegister():
    ...
    if form.validate():
        ...
        if (user_name_count == 0) and (user_email_count == 0):
            ...
            current_user = User( form.username.data, form.useremail.data, form.password.data)
...
def upload():
    ...
    if request.method == 'POST':
        ...
        for f in file_obj:
            ...
            flash('{src} uploaded to S3 as {safe_name}'.format(src=form.file_selector.data.filename, safe_name=filename))
        return "uploading..."

    return render_template('drop_box.html',form = form)
