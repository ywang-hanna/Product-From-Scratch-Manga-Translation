...
import boto3
import os
os.environ["GOOGLE_APPLICATION_CREDENTIALS"]="product-analytics-group7/server/nmt-API-07b7802bb743.json"
import main_func
...
# model, model2 = main_func.model_load('product-analytics-group7/server/checkpoint')
...
def upload():
    ...
    if request.method == 'POST':
        ...
        for f in file_obj:
            ...
            os.path.join(tmp2, "trn_" + filename))

            flash('{safe_name} is translated! You can download now :D'
...
def download():
    ...
    def f(x): return(secure_filename(x.split('/')[-1]))
    ...
    print(curr_files)
    print([secure_filename(x) for x in available_files])
    
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        for curr_file in curr_files:
            if 'trn_'+curr_file in [secure_filename(x) for x in available_files]:
